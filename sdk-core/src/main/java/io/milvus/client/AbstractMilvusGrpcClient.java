...
public abstract class AbstractMilvusGrpcClient implements MilvusClient {
    ...
    private void removeTsCache(String databaseName, String collectionName) {
        CollectionTsCache.getInstance().invalidate(
                currentEndpoint(), actualDbName(databaseName), collectionName);
    }

    private void updateTsCache(String databaseName, String collectionName, long timestamp) {
        CollectionTsCache.getInstance().set(
                currentEndpoint(), actualDbName(databaseName), collectionName, timestamp);
    }
    ...
    public R<RpcStatus> createCollection(CreateCollectionParam requestParam) {
        ...
        try {
            ...
            handleResponse(title, response);
            removeCollectionCache(requestParam.getDatabaseName(), requestParam.getCollectionName());
            return R.success(new RpcStatus(RpcStatus.SUCCESS_MSG));
        } catch (StatusRuntimeException e) {
            logError("{} RPC failed! Exception:{}", title, e);
            return R.failed(e);
    ...
    public R<RpcStatus> dropCollection(DropCollectionParam requestParam) {
        ...
        try {
            ...
            removeTsCache(dbName, collectionName);
            return R.success(new RpcStatus(RpcStatus.SUCCESS_MSG));
        } catch (StatusRuntimeException e) {
            logError("{} RPC failed! Exception:{}", title, e);
            return R.failed(e);
    ...
    public R<RpcStatus> renameCollection(RenameCollectionParam requestParam) {
        ...
        try {
            ...
            removeCollectionCache(oldDbName, requestParam.getOldCollectionName());
            removeCollectionCache(newDbName, requestParam.getNewCollectionName());
            CollectionTsCache.getInstance().move(currentEndpoint(), oldDbName,
    ...
    public R<RpcStatus> createAlias(CreateAliasParam requestParam) {
        ...
        try {
            ...
            CollectionTsCache.getInstance().copy(currentEndpoint(),
    ...
    public R<RpcStatus> dropAlias(DropAliasParam requestParam) {
        ...
        try {
            ...
            handleResponse(title, response);
            removeCollectionCache(requestParam.getDatabaseName(), requestParam.getAlias());
            removeTsCache(requestParam.getDatabaseName(), requestParam.getAlias());
            return R.success(new RpcStatus(RpcStatus.SUCCESS_MSG));
        } catch (StatusRuntimeException e) {
            logError("{} RPC failed! Exception:{}", title, e);
            return R.failed(e);
        ...
        Futures.addCallback(
                ...
                new FutureCallback<MutationResult>() {
                    ...
                    public void onSuccess(MutationResult result) {
                        if (result.getStatus().getErrorCode() == ErrorCode.SchemaMismatch) {
                            removeCollectionCache(dbName, collectionName);
                        }
                        if (result.getStatus().getErrorCode() == ErrorCode.Success) {
                            ...
                            // update the last write timestamp for SESSION consistency
                            updateTsCache(dbName, collectionName, result.getTimestamp());
                        } else {
    ...
    public R<QueryIterator> queryIterator(QueryIteratorParam requestParam) {
        ...
        QueryIterator queryIterator = new QueryIterator(requestParam,
                createIteratorRpcStub(requestParam.getDatabaseName()),
    ...
    public R<SearchIterator> searchIterator(SearchIteratorParam requestParam) {
        ...
        DescCollResponseWrapper descCollResponseWrapper = new DescCollResponseWrapper(descResp.getData());
        // for MilvusClientV1, we don't support to set rpcDeadlineMs for iterator, rpcDeadlineMs is always 0(no deadline)
        SearchIterator searchIterator = new SearchIterator(requestParam,
                createIteratorRpcStub(requestParam.getDatabaseName()),
                descCollResponseWrapper.getPrimaryField());
